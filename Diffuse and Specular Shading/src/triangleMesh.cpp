...
//Really just copied and pasted from the method above. Modified to return an empty 
//point rather than false, and to check whether each intersection is nearer than the 
//last
intersectionPoint triangleMesh::intersect(const ray& r) const{

  //Establish empty point for bail/nonintersection/comparison
  intersectionPoint intPoint; 

  if(!_boundingBox.isHit(r)) return intPoint;
  ...
  for(std::vector<triangle>::const_iterator itr = _triangles.begin(); itr != _triangles.end(); itr++){
    ...
    float t;
    vec3d barycentric;
    if(itr->intersect(r, barycentric, t)){

      //Check to see if closer (thanks for including the <> signs, they made this code/algorithm a lot tidier)
      if (intPoint > t){
        intPoint = intersectionPoint(r, barycentric, t, *itr, _material);
    }
  }
}
