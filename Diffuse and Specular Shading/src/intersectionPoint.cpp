...
color intersectionPoint::evaluate(const lightsource_base& ls) const
{
  ...
  vec3d lsVector = ls.getDirection(point());
  color lsIntensity = ls.getIntensity(lsVector, point()); 

  ray intPointRay = getRay(); 

  //Multiply the two vectors by -1 to reverse the direction
  return (lsIntensity * _material.evaluate(-1.0 * lsVector, normal(), -1.0 * intPointRay.direction(), textureCoordinate()));  
